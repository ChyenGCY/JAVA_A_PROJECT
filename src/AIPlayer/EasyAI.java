...
public class EasyAI {
        ...
        for(int i = 0;i<8;i++){
            for(int j = 0 ;j<8;j++){
                if(GameRule.getExpectMatrix()[i][j]!=0){
                    ...
                    if ((i == 0 && j == 0 )||( i == 0 && j == 8 - 1) ||( i == 8 - 1 && j == 8 - 1) ||( i == 8 - 1 && j == 0)){
                        step_cordinate[0] = i;
                        step_cordinate[1] = j;
    ...
    public int max_point(int x,int y,int[][] mapnow,int[][] expectnow,int depin,int depinmax){
		...
		for (int j = 0; j < 8; ++j)
		{
            // expectnow[k][l] = GameRule.virtual_numCanFlip(k, l,-1*getChessPiece().getType(), newMap);
			expectnow2[i][j] =  GameRule.virtual_numCanFlip(i, j, -1*getChessPiece().getType(), mapnow2);				//预判对方是否可以走棋
			if (expectnow2[i][j]!=0)
			{
				++MAXEXPECT;
				if ((i == 0 && j == 0) || (i == 0 && j == 8 - 1) || (i == 8 - 1 && j == 8 - 1) || (i == 8 - 1 && j == 0))return -1800;	//如果对方有占角的可能
				if ((i < 2 && j < 2) || (i < 2 && 8 - j - 1 < 2) || (8 - 1 - i < 2 && j < 2) || (8 - 1 - i < 2 && 8 - 1 - j < 2))++LINEEXPECT;
			}
		}
	if (LINEEXPECT * 10 > MAXEXPECT * 7)return 1800;     //如果对方走到坏点状态较多 剪枝
